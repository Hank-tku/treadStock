...
class _StrategyDetailPageState extends ConsumerState<StrategyDetailPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        onRefresh: () => ref.read(strategyDetailProvider.notifier).loadDetail(widget.strategyId),
        child: ListView(
          children: [
            _buildHeader(state),
            if (state.isLoading)
                ...
                if (strategy != null && !strategy.isDefault)
                  IconButton(
                    ...
                    icon: _isDeleting
                        ? const SizedBox(
                            width: 18, height: 18,
                            child: CircularProgressIndicator(strokeWidth: 2, color: StockColors.danger))
                        : const Icon(Icons.delete_outline, size: 20, color: StockColors.danger),
                    constraints: const BoxConstraints(minWidth: 44, minHeight: 44),
                  ),
                IconButton(
                  onPressed: () => context.push('/strategy/${widget.strategyId}/edit'),
                  icon: const Icon(Icons.edit_outlined, size: 20, color: StockColors.brand),
                  constraints: const BoxConstraints(minWidth: 44, minHeight: 44),
                ),
              ],
            ),
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          const Text('统计计算失败', style: TextStyle(fontSize: 13, color: StockColors.textSecondary)),
          const SizedBox(height: 4),
          GestureDetector(
            onTap: () => ref.read(strategyDetailProvider.notifier).loadDetail(widget.strategyId),
            child: const Text('重新计算', style: TextStyle(fontSize: 13, color: StockColors.brand)),
          ),
        ],
      ),
    );
  ...
  Widget _buildReviewBanner(StrategyDetailState state) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          const Icon(Icons.notifications_active, color: StockColors.warning, size: 20),
          const SizedBox(width: 8),
          const Expanded(child: Text('该策略已运行满30个交易日，建议进行复盘', style: AppTextStyles.body)),
          TextButton(onPressed: () => _showReviewDialog(state), child: const Text('立即复盘')),
        ],
      ),
    );
  }
  ...
  Widget _buildStatsCards(StrategyDetailState state) {
    ...
    return Padding(
      ...
      child: Row(
        children: [
          _buildStatCard('命中率', stats.hitRateDisplay,
              getScoreColor(_hitRateToScore(stats.hitRate))),
          const SizedBox(width: 8),
          _buildStatCard('极限分值', stats.extremeScoreDisplay, StockColors.textPrimary),
          const SizedBox(width: 8),
          _buildStatCard('健康度', stats.healthScoreDisplay,
      ...
      children: [
        Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              Text('共 ${state.hitRecords.length} 条', style: AppTextStyles.caption),
            ],
          ),
        ),
        if (state.hitRecords.isEmpty)
          const Padding(
            ...
            child: Text('暂无推荐记录', style: TextStyle(fontSize: 13, color: StockColors.gray500)),
          )
        else
          ...state.hitRecords.take(20).map((record) => _buildHitRecordItem(record)),
      ],
    );
  }
  ...
  Widget _buildHitRecordItem(StrategyHitRecord record) {
    ...
    return InkWell(
      onTap: () => context.push('/stock/${record.stockCode}', extra: {
      ...
      child: Container(
        ...
        decoration: const BoxDecoration(border: Border(bottom: BorderSide(color: StockColors.border))),
        child: Row(
          children: [
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text('${record.stockCode}  ${record.recommendDate}', style: AppTextStyles.caption),
                ],
              ),
            ),
            Text('评分 ${record.recommendScore}', style: AppTextStyles.numberSm),
            const SizedBox(width: 12),
            Text(record.actualChangeDisplay,
  ...
  Widget _buildSuggestions(StrategyDetailState state) {
    return Column(
      ...
      children: [
        const Padding(
          padding: EdgeInsets.fromLTRB(AppTheme.pagePadding, 16, AppTheme.pagePadding, 8),
          child: Text('迭代建议', style: AppTextStyles.h2),
        ),
        ...state.suggestions.map((s) => Container(
              margin: const EdgeInsets.symmetric(horizontal: AppTheme.pagePadding, vertical: 4),
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(
                color: StockColors.bgSecondary,
                borderRadius: BorderRadius.circular(AppTheme.radiusMd),
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(s.condition, style: AppTextStyles.body.copyWith(fontWeight: FontWeight.w500)),
                  const SizedBox(height: 4),
    ...
    return Column(
      ...
      children: [
          ...
          child: Row(
            ...
            children: [
              ...
              if (state.reviewHistory.isNotEmpty)
                Text('${state.reviewHistory.length} 次', style: AppTextStyles.caption),
            ],
          ),
        ),
        if (state.reviewHistory.isEmpty)
          const Padding(
            padding: EdgeInsets.all(AppTheme.pagePadding),
            child: Text('暂无复盘记录', style: TextStyle(fontSize: 13, color: StockColors.gray500)),
          )
        else
        ...
        children: [
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(Formatters.formatDate(review.reviewDate), style: AppTextStyles.body),
                if (review.note != null && review.note!.isNotEmpty)
                  Text(review.note!, style: AppTextStyles.caption, maxLines: 1, overflow: TextOverflow.ellipsis),
              ],
            ),
          ),
          Text('健康度 ${review.healthScore.toStringAsFixed(1)}', style: AppTextStyles.numberSm),
        ],
      ),
    );
  }
  ...
  Future<void> _handleDelete() async {
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            ...
            child: const Text('删除', style: TextStyle(color: StockColors.danger)),
          ),
        ],
      ),
    );
    ...
    final success = await ref.read(strategyListProvider.notifier).deleteStrategy(widget.strategyId);
    if (!mounted) return;
  ...
  void _showReviewDialog(StrategyDetailState state) {
    ...
    ref.read(strategyDetailProvider.notifier).generateChecklist(widget.strategyId);
                ...
                child: Column(
                  ...
                  children: [
                    if (detailState.checklist.isEmpty)
                      ...
                      ...detailState.checklist.map((item) => _buildChecklistItem(item)),
