...
### 新增
- **K 线缓存维度升级**：缓存按股票代码、市场和周期区分，避免 60/120/250 天数据互相覆盖
- **设置面板增强**：菜单改为设置式侧边面板，支持主题切换和清理缓存入口

### 修复
- **推荐页冷启动 loading 过久**：首屏只评分有限候选，避免模拟器被大量 K 线 fallback 请求拖住
- **高频页面复用缓存**：推荐、关注、详情和市场环境改用缓存优先的数据入口

### 变更
- 详情页新闻改为主图和评分加载完成后再请求，不阻塞首屏
